...
```
POST   /api/sessions/[id]/workouts                  → { exercises: WorkoutEntry[], dirtyExerciseIds?: string[], removedExerciseIds?: string[] } → { data: WorkoutLog[], newBadges, autoGeneratedPostIds }  (201)
...
`/api/trainer/workouts` callers rely on this. Genuine concurrent edits to the
_same_ exercise still resolve last-write-wins (acceptable; rare).
