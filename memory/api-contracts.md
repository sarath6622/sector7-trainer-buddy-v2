...
> are excluded from name/photo panels but counted in anonymous aggregates.
...
When `ClientProfile.showOnTv = false`, the client is omitted from every row
above. They still count toward `liveNow.count` (anonymously) and toward branch
aggregates that may be added in v2.
