...
**Amendment (2026-06-12, same day):** the "per-exercise, not per-set" caveat bit in production within hours — trainer and client routinely edit the _same_ exercise concurrently (client ticks a set / rest autofill while the trainer types the next set), and the stale copy's save deleted the peer's newly typed set. Two follow-up changes:
