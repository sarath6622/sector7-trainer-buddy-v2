...
interface TodayItem {
  class: KickboxingClass;
  session: KickboxingSession | null;
}
...
export default function KickboxingTrainerPage() {
  ...
  const [allClasses, setAllClasses] = useState<KickboxingClass[]>([]);

  // ── Attendance mode state ──
  const [activeClass, setActiveClass] = useState<KickboxingClass | null>(null);
  ...
  const fetchToday = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/kickboxing/sessions/today');
  ...
  function enterAttendanceMode(cls: KickboxingClass, date: string) {
    setActiveClass(cls);
    setActiveDate(date);
    setMode('attendance');
    setShowSearch(false);
  ...
  const openSession = useCallback(async (classId: string, date: string) => {
    ...
    setAttendance([]);
    try {
      const res = await fetch('/api/kickboxing/sessions', {
        method: 'POST',
  ...
  useEffect(() => {
    if (mode === 'attendance' && activeClass) {
      ...
      fetch(`/api/kickboxing/classes/${activeClass.id}/enrollments`)
        .then((r) => r.json())
  ...
  const fetchAttendance = useCallback(async (sid: string) => {
    ...
    try {
      const res = await fetch(`/api/kickboxing/sessions/${sid}/attendance`);
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/kickboxing/clients/search?${params}`);
        const data = await res.json();
  ...
  async function handleStart(item: TodayItem) {
    ...
    try {
      const sessionRes = await fetch('/api/kickboxing/sessions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const startRes = await fetch(`/api/kickboxing/sessions/${sid}/start`, { method: 'POST' });
      if (!startRes.ok) {
  ...
  async function markPresent(member: EnrolledMember) {
    ...
    try {
      ...
      const res = await fetch(`/api/kickboxing/sessions/${sessionId}/attendance`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function removeAttendance(record: AttendanceRecord) {
    if (!sessionId) return;
    try {
      await fetch(`/api/kickboxing/sessions/${sessionId}/attendance/${record.id}`, {
        method: 'DELETE',
      });
      await fetchAttendance(sessionId);
    } catch {
  ...
  async function handleEndSession() {
    ...
    try {
      const res = await fetch(`/api/kickboxing/sessions/${sessionId}/end`, { method: 'POST' });
      const data = await res.json();
      ...
      <div className="flex flex-col min-h-full">
        ...
        <div className="px-4 pt-4 pb-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-lg font-bold">Kickboxing</h1>
          </div>
                ...
                >
                  ...
                  <div className="flex items-start gap-3 mb-4">
                    <div
                      className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${
                        isInProgress ? 'bg-emerald-500/15' : 'bg-red-500/10'
                      }`}
                    >
                      <Activity
                        className={`h-5 w-5 ${isInProgress ? 'text-emerald-500' : 'text-red-500'}`}
                      />
                    </div>
                  ...
                  {isCompleted ? (
                    ...
                    <button
                      ...
                      disabled={isStarting}
                      className="w-full flex items-center justify-center gap-2 rounded-xl bg-red-500 py-3 text-sm font-semibold text-white hover:bg-red-600 transition-colors disabled:opacity-50"
                    >
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      {!sessionLoading && sessionStatus === 'SCHEDULED' && (
        <div className="mx-4 mt-3 rounded-2xl bg-red-500/10 ring-1 ring-red-500/25 p-4">
          <p className="text-sm font-semibold text-red-500 mb-0.5">Class not started yet</p>
          ...
          <button
            onClick={async () => {
              if (!sessionId) return;
              const res = await fetch(`/api/kickboxing/sessions/${sessionId}/start`, {
                method: 'POST',
              });
              if (res.ok) setSessionStatus('IN_PROGRESS');
              else toast.error('Failed to start class');
            }}
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-red-500 py-2.5 text-sm font-semibold text-white hover:bg-red-600 transition-colors"
          >
            <Play className="h-4 w-4" />
            Start Class
          </button>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {!isMarked && (
                      <p className="text-xs text-muted-foreground">
                        {member.clientType === 'GYM_MEMBER' ? 'PT Client' : 'External'}
                      </p>
                    )}
                  </div>
