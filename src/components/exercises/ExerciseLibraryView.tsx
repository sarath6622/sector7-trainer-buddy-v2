...
interface ExerciseLibraryViewProps {
  /** Base API path — e.g. '/api/admin/exercises' or '/api/trainer/exercises'.
   *  The component derives the [id] and bulk-import endpoints from this. */
  basePath: string;
}

export function ExerciseLibraryView({ basePath }: ExerciseLibraryViewProps) {
  ...
  const fetchExercises = useCallback(
    ...
    [basePath, search, categoryFilter, muscleFilter],
  );
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = editingExercise ? `${basePath}/${editingExercise.id}` : basePath;
  ...
  async function handleDelete(exerciseId: string) {
    ...
    if (!ok) return;
    const res = await fetch(`${basePath}/${exerciseId}`, { method: 'DELETE' });
    if (res.ok) {
      fetchExercises(pagination.page);
    } else {
      const err = await res.json();
  ...
  async function handleBulkImport() {
    ...
    try {
      ...
      const res = await fetch(`${basePath}/bulk-import`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
